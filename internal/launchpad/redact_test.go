...
func TestRedactReportRemovesIdentityAndCredentialLikeData(t *testing.T) {
	report := Report{
		Snapshot: &Snapshot{Hostname: "PRIVATE-HOST", TargetUser: "alice", SSHPasswordAuthentication: true, Tailscale: TransportState{IP: "fd7a:115c:a1e0::1"}},
	...
	if !redacted.Snapshot.SSHPasswordAuthentication {
		t.Fatal("redaction corrupted non-secret authentication policy evidence")
	}
